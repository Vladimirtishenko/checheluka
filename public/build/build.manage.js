	    ...
	    _createClass(Templates, [{
	        ...
	        value: function orders() {
	            ...
	            function templates(data) {
	                ...
	                var templates = '<form class="a-privat-table_bought">' + '<input name="_id" type="hidden" value="' + data._id + '" />' + '<div class="a-privat-table_bought__number">' + '<p> <span>Заказ </span>№ ' + data.orderNumber + '<span> от </span> ' + constructorDate + ' <a class="che-print-image" href="/order/print/' + data._id + '">Другие действия</a></p>' + '<p>На сумму: <span class="a-common-price-' + data._id + '">' + data.priceCommon + ' руб.</span></p>' + '</div>' + '<div class="a-privat-table a-privat-table__bought a-privat-table__orders">' + '<div class="a-privat-table__info_order">' + '<h5>Заказ:</h5>' + '<table>' + '<tr cols="2">' + '<th></th>' + '<th>Наименование</th>' + '<th>Размер</th>' + '<th>Цвет</th>' + '<th>Количество</th>' + '<th>Цена</th>' + '</tr>' + ordersOne(data.goods, data._id) + '</table>' + '<div class="a-status-block">' + '<h5 class="a-status-orders"><span>Статус:<span></h5>' + '<select class="a-privat-table__changed" name="status">' + statusChecker(data.status) + '</select>' + '</div>' + '</div>' + '<div class="a-privat-table__user">' + '<h5>Информация о заказе:</h5>' + '<table>' + '<tr>' + '<th>Страна</th>' + '<td><input name="country" type="text" value="' + decodeURIComponent(data.country) + '" /></td>' + '</tr>' + '<tr>' + '<th>Город</th>' + '<td><input name="sity" type="text" value="' + decodeURIComponent(data.sity) + '" /></td>' + '</tr>' + '<tr>' + '<th>Служба доставки</th>' + '<td><input name="delivery" type="text" value="' + decodeURIComponent(data.delivery) + '" /></td>' + '</tr>' + '<tr>' + '<th>Склад</th>' + '<td><input name="warehouse" type="text" value="' + decodeURIComponent(data.warehouse) + '" /></td>' + '</tr>' + '</table>' + '<h5>Информация о клиенте:</h5>' + '<table>' + '<tr>' + '<th>ФИО</th>' + '<td>' + decodeURIComponent(data.fio) + '</td>' + '</tr>' + '<tr>' + '<th>Номер телефона</th>' + '<td>' + decodeURIComponent(data.number) + '</td>' + '</tr>' + '<tr>' + '<th>Почта</th>' + '<td>' + decodeURIComponent(data.email) + '</td>' + '</tr>' + '</table>' + '</div>' + '</div>' + '<div class="a-saved-status">' + '<button type="submit" class="a-privat-table__submit">Сохранить</button>' + '</div>' + '</form>';
