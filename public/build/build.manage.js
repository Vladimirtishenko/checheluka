...
/***/ function(module, exports, __webpack_require__) {
	...
	var _add_or_delete_action = __webpack_require__(81);

	var _add_or_delete_action2 = _interopRequireDefault(_add_or_delete_action);

	var _all_goods_load = __webpack_require__(82);
	...
	window.addEventListener('DOMContentLoaded', function () {
		...
		new _auction_reload2.default(document.querySelector('.a-reload-auction'));
		new _user_controls2.default(document.querySelector('.che-users-view'));
		new _order_details2.default(document.querySelector('.che-items-print-controls'));
		new _auction_goods_remove2.default(document.querySelector('.a-fuck-auction-remove'));
	});
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _privat_change_status_order = __webpack_require__(83);

	var _privat_change_status_order2 = _interopRequireDefault(_privat_change_status_order);

	var _templates = __webpack_require__(85);
	...
	var AsyncLoadFromAnouterResourse = function (_Helper) {
		...
		function AsyncLoadFromAnouterResourse(el) {
			...
			_this.searchSelect = document.querySelector('#a-search-admin__select');
			...
			if (_this.searchSelect) {
				_this.flyEvent('add', ['change'], [_this.searchSelect], _this.handlerToSearchFilters.bind(_this));
			}
		...
		_createClass(AsyncLoadFromAnouterResourse, [{
			...
			key: 'handlerToSearchFilters',
			value: function handlerToSearchFilters(event) {
				...
				if (this.action == 'orders') {
					...
					try {
						for (var _iterator2 = obj[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
							...
							tmp += this.templates(i._id, i.img || i.src, i.title, i.description, i.size, i.color, i.Material || i.material, i.Sostav || i.consistOf, i.countInWarehouse, i.priority, i.PriceRoz || i.price, i.auctionPrice, i.artikul || i.art, i.unsoldCount || 0);
						}
					} catch (err) {
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _notifier = __webpack_require__(84);
	...
	var ChangeStatus = function (_Helper) {
		...
		function ChangeStatus(elem) {
			...
			_this.view = elem;
			_this.Notification = new _notifier2.default();
			_this.savedVerify = ['_id', 'status', 'country', 'sity', 'delivery', 'warehouse'];

			_this.flyEvent('add', ['click'], [_this.view], _this.removeItem.bind(_this));
			_this.flyEvent('add', ['submit'], [_this.view], _this.submitForm.bind(_this));
		...
		_createClass(ChangeStatus, [{
			...
			key: 'responseAfterDelete',
			value: function responseAfterDelete(parent, obj) {
				try {
					...
					var commonPrice = json.priceUpdated,
			...
			key: 'responseAfterUpdate',
			value: function responseAfterUpdate(obj) {
				try {
					var json = JSON.parse(obj);
...
/***/ function(module, exports) {
	...
	var Notifier = function () {
		...
		_createClass(Notifier, [{
			...
			key: 'notifyClose',
			value: function notifyClose(event) {
	    ...
	    _createClass(Templates, [{
	        ...
	        value: function orders() {
	            ...
	            function templates(data) {
	                ...
	                var constructorDate = new Date(data.date).toLocaleDateString(),
	                    statusText = ['Не оплачен', 'Оплачен', 'Выполнен', 'Отменен', 'Обработан'];

	                var templates = '<form class="a-privat-table_bought">' + '<input name="_id" type="hidden" value="' + data._id + '" />' + '<div class="a-privat-table_bought__number">' + '<p>' + '<span>Заказ </span>№ ' + data.orderNumber + '<span> от </span> ' + constructorDate + ' <a class="che-print-image" target="_blank" href="/order/print/' + data._id + '">Другие действия</a></p><span class="a-status-wrapper">Статус: <i class="a-status-' + data.status + '">' + statusText[data.status] + '</i></span>' + '<p>На сумму: <span class="a-common-price-' + data._id + '">' + data.priceCommon + ' руб.</span></p>' + '</div>' + '<input type="checkbox" class="a-show-or-hide-order" value="Открыть" />' + '<div class="a-orders-description a-orders-hide-block"><div class="a-privat-table a-privat-table__bought a-privat-table__orders">' + '<div class="a-privat-table__info_order">' + '<h5>Заказ:</h5>' + '<table>' + '<tr cols="2">' + '<th></th>' + '<th>Наименование</th>' + '<th>Размер</th>' + '<th>Цвет</th>' + '<th>Количество</th>' + '<th>Цена</th>' + '</tr>' + ordersOne(data.goods, data._id) + '</table>' + '<div class="a-status-block">' + '<h5 class="a-status-orders"><span>Статус:<span></h5>' + '<select class="a-privat-table__changed" name="status">' + statusChecker(data.status) + '</select>' + '</div>' + '</div>' + '<div class="a-privat-table__user">' + '<h5>Информация о заказе:</h5>' + '<table>' + '<tr>' + '<th>Страна</th>' + '<td><input name="country" type="text" value="' + decodeURIComponent(data.country) + '" /></td>' + '</tr>' + '<tr>' + '<th>Город</th>' + '<td><input name="sity" type="text" value="' + decodeURIComponent(data.sity) + '" /></td>' + '</tr>' + '<tr>' + '<th>Служба доставки</th>' + '<td><input name="delivery" type="text" value="' + decodeURIComponent(data.delivery) + '" /></td>' + '</tr>' + '<tr>' + '<th>Склад</th>' + '<td><input name="warehouse" type="text" value="' + decodeURIComponent(data.warehouse) + '" /></td>' + '</tr>' + '</table>' + '<h5>Информация о клиенте:</h5>' + '<table>' + '<tr>' + '<th>ФИО</th>' + '<td>' + decodeURIComponent(data.fio) + '</td>' + '</tr>' + '<tr>' + '<th>Номер телефона</th>' + '<td>' + decodeURIComponent(data.number) + '</td>' + '</tr>' + '<tr>' + '<th>Почта</th>' + '<td>' + decodeURIComponent(data.email) + '</td>' + '</tr>' + '</table>' + '</div>' + '</div>' + '<div class="a-saved-status">' + '<button type="submit" class="a-privat-table__submit">Сохранить</button>' + '</div>' + '</div>' + '</form>';
	                ...
	                function ordersOne(goods, id) {
	                    ...
	                    for (var key in goods) {
	                        ...
	                        price += +goods[key].price, color = decodeURIComponent(goods[key].color).replace("+", " + ");

	                        tmp += '<tr>' + '<td data-number="' + key + '" data-id="' + id + '" class="a-remove-item-order">X</td>' + '<td>' + decodeURIComponent(goods[key].art) + ' / ' + decodeURIComponent(goods[key].title) + '</td>' + '<td>' + decodeURIComponent(goods[key].size) + '</td>' + '<td>' + color + '</td>' + '<td>' + goods[key].count + '</td>' + '<td>' + goods[key].price + '</td>' + '</tr>';
	                    }
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _flatpickr = __webpack_require__(87);
	...
	var _error = __webpack_require__(69);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var AuctionReload = function (_Helper) {
		_inherits(AuctionReload, _Helper);

		function AuctionReload(elem) {
			_classCallCheck(this, AuctionReload);

			var _this = _possibleConstructorReturn(this, (AuctionReload.__proto__ || Object.getPrototypeOf(AuctionReload)).call(this));
			...
			_this.flyEvent('add', ['click'], [_this.button], _this.clickToReload.bind(_this));
			return _this;
		}

		_createClass(AuctionReload, [{
			key: 'clickToReload',
			value: function clickToReload(event) {
			...
			key: 'responseDate',
			value: function responseDate(obj) {
...
/***/ function(module, exports, __webpack_require__) {
	...
	var UserControls = function (_Helper) {
		_inherits(UserControls, _Helper);

		function UserControls(el) {
			...
			_this.flyEvent('add', ['click'], [el], _this.handlerToRemove.bind(_this));
			_this.Notifier = new _notifier2.default();
		...
		_createClass(UserControls, [{
			key: 'handlerToRemove',
			value: function handlerToRemove(event) {
			...
			key: 'responseHandler',
			value: function responseHandler(closest, obj) {
				try {
					var json = JSON.parse(obj);
					if (json.status != 200) {
						throw json.errmsg;
...
/***/ function(module, exports, __webpack_require__) {
	...
	var OrdersDetails = function (_Helper) {
		_inherits(OrdersDetails, _Helper);

		function OrdersDetails(el) {
			_classCallCheck(this, OrdersDetails);

			var _this = _possibleConstructorReturn(this, (OrdersDetails.__proto__ || Object.getPrototypeOf(OrdersDetails)).call(this));

			_this.el = el;
			_this.blockForm = document.querySelector('.a-form-to-send-details');
			_this.access = ['subject', 'html_msg', 'ttn', 'order', 'email', 'status', 'orderId'];
			_this.Notification = new _notifier2.default();

			_this.flyEvent('add', ['click'], [_this.el], _this.tabChange.bind(_this));
			_this.flyEvent('add', ['submit'], [_this.blockForm], _this.submitMessage.bind(_this));

			return _this;
		}

		_createClass(OrdersDetails, [{
			key: 'tabChange',
			...
			key: 'messageNotify',
			value: function messageNotify(obj) {
...
/***/ function(module, exports, __webpack_require__) {
	...
	var RemoveGoodsAll = function (_Helper) {
		_inherits(RemoveGoodsAll, _Helper);

		function RemoveGoodsAll(el) {
			_classCallCheck(this, RemoveGoodsAll);

			var _this = _possibleConstructorReturn(this, (RemoveGoodsAll.__proto__ || Object.getPrototypeOf(RemoveGoodsAll)).call(this));

			if (!el) return _possibleConstructorReturn(_this);

			_this.el = el;
			_this.Notification = new _notifier2.default();
			_this.view = document.querySelector('.a-all-goods-table');

			_this.flyEvent('add', ['click'], [_this.el], _this.removeAllHandler.bind(_this));

			return _this;
		}

		_createClass(RemoveGoodsAll, [{
			key: 'removeAllHandler',
			value: function removeAllHandler(event) {

				if (!event && !event.target) return;

				var promt = prompt('Ключевое слово?');
			...
			key: 'responseAfterDelete',
			value: function responseAfterDelete(obj) {
				try {
					var json = JSON.parse(obj);
