	    ...
	    _createClass(Templates, [{
	        ...
	        value: function orders() {
	            ...
	            function templates(data) {
	                ...
	                var templates = '<div class="a-privat-table_bought">' + '<div class="a-privat-table_bought__number">' + '<p> <span>Заказ </span>№ ' + data.orderNumber + '<span> от </span> ' + constructorDate + ' <a class="che-print-image" href="/order/print/' + data._id + '">Печать</a></p>' + '<p>На сумму: <span>' + data.priceCommon + ' руб.</span></p>' + '</div>' + '<div class="a-privat-table a-privat-table__bought a-privat-table__orders">' + '<div class="a-privat-table__info_order">' + '<h5>Заказ:</h5>' + '<table>' + '<tr>' + '<th>Наименование</th>' + '<th>Размер</th>' + '<th>Цвет</th>' + '<th>Количество</th>' + '<th>Цена</th>' + '</tr>' + ordersOne(data.goods) + '</table>' + '<h5 class="a-status-orders">Статус: <span>' + (data.status == 0 ? 'Не оплачен' : data.status == 1 ? 'Оплачен' : "Отменен") + '<span></h5>' + '<select class="a-privat-table__changed" name="status">' + '<option value="0">' + 'Не оплачен' + '</option>' + '<option value="1">' + 'Оплачен' + '</option>' + '<option value="2">' + 'Отменен' + '</option>' + '</select>' + '<button class="a-privat-table__submit" data-value="' + data.orderNumber + '">Сохранить статус</button>' + '</div>' + '<div class="a-privat-table__user">' + '<h5>Информация о заказе:</h5>' + '<table>' + '<tr>' + '<th>Страна</th>' + '<td>' + decodeURIComponent(data.country) + '</td>' + '</tr>' + '<tr>' + '<th>Город</th>' + '<td>' + decodeURIComponent(data.sity) + '</td>' + '</tr>' + '<tr>' + '<th>Служба доставки</th>' + '<td>' + decodeURIComponent(data.delivery) + '</td>' + '</tr>' + '<tr>' + '<th>Склад</th>' + '<td>' + decodeURIComponent(data.warehouse) + '</td>' + '</tr>' + '</table>' + '<h5>Информация о клиенте:</h5>' + '<table>' + '<tr>' + '<th>ФИО</th>' + '<td>' + decodeURIComponent(data.fio) + '</td>' + '</tr>' + '<tr>' + '<th>Номер телефона</th>' + '<td>' + decodeURIComponent(data.number) + '</td>' + '</tr>' + '<tr>' + '<th>Почта</th>' + '<td>' + decodeURIComponent(data.email) + '</td>' + '</tr>' + '</table>' + '</div>' + '</div>' + '</div>';
	                ...
	                function ordersOne(goods) {
	                    ...
	                    for (var key in goods) {
	                        ...
	                        tmp += '<tr>' + '<td>' + decodeURIComponent(goods[key].art) + ' / ' + decodeURIComponent(goods[key].title) + '</td>' + '<td>' + decodeURIComponent(goods[key].size) + '</td>' + '<td>' + decodeURIComponent(goods[key].color) + '</td>' + '<td>' + goods[key].count + '</td>' + '<td>' + goods[key].price + '</td>' + '</tr>';
	                    }
