...
/***/ function(module, exports, __webpack_require__) {
	...
	var Sockets = function (_Helper) {
		...
		function Sockets() {
			...
			var dateT = new Date();
			_this.socket.emit('setClientTime', { time: dateT.getTime() });
		...
		_createClass(Sockets, [{
			...
			key: 'getCurrentTime',
			value: function getCurrentTime(action, callback) {
				this.registeredCallback[action] = callback;
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _bucket = __webpack_require__(74);
	...
	var AsyncLoad = function (_Helper) {
		...
		_createClass(AsyncLoad, [{
			...
			value: function getCurrentAuction(response) {
				...
				this.pretendentsAuction = response.data.pretendents || null, this.finishedTime = response.data.finishedTime;
			...
			key: 'timerStarted',
			value: function timerStarted() {
				var _this2 = this;

				var timer = document.querySelector('.a-times-frontend');
				var time = Math.round((this.finishedTime - (new Date().getTime() + $app.timeDiff * 1000)) / 1000);
